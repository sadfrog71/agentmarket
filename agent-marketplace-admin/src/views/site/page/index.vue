  ...
  <div class="app-container site-page-manager">
    <section class="site-banner">
      <div>
        ...
        <h2>官网菜单管理</h2>
        <span>在这里维护官网菜单、栏目和页面文案；保存草稿不影响访客，发布时才切换公开版本。</span>
      </div>
      <div class="site-banner-rules"><span>保存草稿</span><b>→</b><span>核对内容</span><b>→</b><span>直接发布</span></div>
      <el-button type="primary" icon="Plus" @click="handleAdd" v-hasPermi="['site:page:add']">新增菜单</el-button>
    </section>

    <el-form ref="queryRef" :model="queryParams" :inline="true" v-show="showSearch" class="query-panel">
      <el-form-item label="菜单名称" prop="title"><el-input v-model="queryParams.title" placeholder="如 首页、智慧水务" clearable @keyup.enter="handleQuery" /></el-form-item>
    ...
    <el-table v-loading="loading" :data="pageList" class="site-table">
      <el-table-column label="菜单名称" min-width="210">
        <template #default="scope"><div class="page-code"><span>{{ (scope.row.title || '页').slice(0, 1) }}</span><div><strong>{{ scope.row.title || '未命名菜单' }}</strong><small>{{ scope.row.routePath }}</small></div></div></template>
      </el-table-column>
      <el-table-column label="访问路径" prop="routePath" min-width="220" />
    ...
    <div v-if="!loading && !pageList.length" class="site-empty"><strong>尚未导入官网菜单</strong><span>迁移工具导入后，所有既有页面会在此统一维护。</span></div>
    ...
    <el-dialog :title="dialogTitle" v-model="open" width="1080px" append-to-body destroy-on-close>
      ...
      <el-form ref="pageRef" :model="form" :rules="rules" label-width="96px">
        <el-row :gutter="18">
          <el-col :span="8"><el-form-item label="内部编码" prop="pageCode"><el-input v-model="form.pageCode" placeholder="如 HOME" :disabled="Boolean(form.pageId)" @input="normalizeCode" /></el-form-item></el-col>
...
const data = reactive({
  form: emptyForm(),
  queryParams: { pageNum: 1, pageSize: 20, title: undefined, routePath: undefined },
  rules: {
...
function handleAdd() { reset(); dialogTitle.value = '新增官网菜单草稿'; open.value = true }
function handleEdit(row) { reset(); getSitePage(row.pageId).then(res => { form.value = { ...emptyForm(), ...res.data }; dialogTitle.value = `编辑官网菜单 · ${res.data.title || '未命名菜单'}`; open.value = true }) }
...
function handlePublish(row) { proxy.$modal.confirm(`确认直接发布「${row.title || '未命名菜单'}」的当前草稿吗？访客将立即读取新版本。`).then(() => publishSitePage(row.pageId)).then(() => { proxy.$modal.msgSuccess('已发布'); getList() }) }
function handleUnpublish(row) { proxy.$modal.confirm(`确认下架「${row.title || '未命名菜单'}」吗？访客将无法继续读取该页面。`).then(() => unpublishSitePage(row.pageId)).then(() => { proxy.$modal.msgSuccess('已下架'); getList() }) }
