-- 企业官网内容域升级 006：仅用于已有环境的一次性、追加式升级。
-- 执行前必须以冻结的发布包运行 tools/corporate-site-import/upgrade_preflight.py；
-- 本脚本故意不使用 IF NOT EXISTS。表或菜单已存在即应中止并调查，而不能静默掩盖结构漂移。
-- 成功后由升级执行人把脚本 ID、SHA-256、发布版本和预检证据写入 site_schema_migration。
set names utf8mb4;
