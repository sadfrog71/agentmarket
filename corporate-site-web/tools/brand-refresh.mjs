...
function replaceWholeMain(source, main) {
  ...
  return source.slice(0, start) + main + source.slice(end + '</main>'.length)
}
...
function publishBlock(code, mainHtml, strategy) {
  ...
  const bodyExpression = strategy === 'home'
    ...
    : `concat(substring(@old_body, 1, @main_start - 1), ${main}, substring(@old_body, @main_end + 7))`
...
if (command === '--update-static') {
  ...
  process.stdout.write(createSql())
} else {
  process.stderr.write('Usage: node tools/brand-refresh.mjs --update-static | --sql\n')
  process.exitCode = 1
}
